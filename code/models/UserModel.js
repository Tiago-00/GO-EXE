...
module.exports.getUserProductById = async function (id) {
    try {
        let sql = "Select prod_nome,prod_descricao,prod_preco, use_pontos " +
            "From utilizadorproduto " +
            "inner join produtos on id_p = up_id_p " +
            "inner join utilizador on id_u = up_id_u "+
            "where up_id_u = $1 ";
        let result = await pool.query(sql, [id]);
        if (result.rows.length > 0)
            return {
